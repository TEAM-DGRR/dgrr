...
export const SignUp = () => {
	...
	const onSubmit = (e: FormEvent) => {
		...
		if (nickname.length >= 2 && regex.test(nickname)) {
			axios
				.get(`${process.env.REACT_APP_API_URL}/member/nickname-check?nickname=${nickname}`)
				.then((res: any) => {
					// 없다면 회원가입 진행
					if (res.data.nicknameExists === 'false') {
						...
						axios
							...
							.then((res: any) => {
								...
								axios
									.get(`${process.env.REACT_APP_API_URL}/member/login?kakaoId=${res.data.kakaoId}`)
									.then((res: any) => {
	...
	const ShowImg = useMemo(() => {
		...
		return (
			<img src={profileImg.thumbnail} alt={profileImg.type} onClick={fileInput} id='ex-profile' />
		);
	}, [profileImg]);
		...
		<div className='SignUp'>
			...
			<form onSubmit={onSubmit} className='signupForm'>
				...
				<label className='label' id='nickname-label'>
					...
					<p style={{ color: '#8E8E90' }}>한글/영어/숫자 최소 2자~최대 12자 가능</p>
					<Input maxlength={12} onChange={onChangeNickname} placeholder={'닉네임을 입력해주세요'} />
					<p style={{ visibility: see ? 'visible' : 'hidden', color: 'red' }}>{checkstate}</p>
				</label>
